###################################################
# Title: visualize multiple dom_ave_prec_rate time series Rscript 
# Author: Gaku YAMANO
# Date: 2025/08/26
###################################################
# load packages
...
for (experiment_name in names(input_files)) {
  ...
  # Store the processed data (precipitation RATE) in the list
  results_list[[experiment_name]] <- data.frame(
    time_hr = alltimes_hr,
    prec_rate = dom_ave_prec_rate_timeseries
  )
}
...
max_y <- max(sapply(results_list, function(df) max(df$prec_rate, na.rm = TRUE)))
x_range <- c(0, max_x)
y_range <- c(0, max_y)
...
plot(NA, NA,
     ...
     xlim = x_range,
     ylim = y_range,
     main = "Domain Average of Precipitation Rate Time Series",
     xlab = "Time [hour]",
     ylab = "Precipitation Rate [mm/hour]",
     yaxt = "n"
)
...
for (i in seq_along(results_list)) {
  ...
  lines(data_to_plot$time_hr, data_to_plot$prec_rate,
        col = plot_colors[i],
        lwd = 2
  )
}
