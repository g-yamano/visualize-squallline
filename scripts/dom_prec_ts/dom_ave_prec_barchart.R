###################################################
# Title: visualize dom_ave_prec barchart Rscript 
# Author: Gaku YAMANO
# Date: 2025/08/23
###################################################
# load packages
...
output_dir <- "../../output/Domain_average_of_precipitation/pdf"
# 2. Specify the output pdf file name
output_filename <- "dom_ave_prec_barchart.pdf"
...
dom_ave_prec_mean_timeseries <- apply(dom_ave_prec, 3, mean, na.rm = TRUE) * 120
...
bar_heights <- tapply(dom_ave_prec_mean_timeseries, time_bins, mean, na.rm = TRUE)
error_bar_values <- tapply(dom_ave_prec_mean_timeseries, time_bins, sd, na.rm = TRUE)
...
bar_centers <- barplot(bar_heights,
                       ylim = c(0, max(bar_heights + error_bar_values) * 1.1),
                       main = "6-Hourly Domain Average of Precipitation",
                       xlab = "Time [hour]",
                       ylab = "Precipitation Rate [kg/m2/s]",
