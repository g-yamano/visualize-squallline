###################################################
# Title: visualize multiple low_level_dom_ave_temp time series Rscript 
# Author: Gaku YAMANO
# Date: 2025/08/26
###################################################
# load packages
...
#################### Setting items ####################
#######################################################
#  Specify the 3D temperature variable name
temp_var_name <- "T" 
...
for (experiment_name in names(input_files)) {
  ...
  # Extract the lowest vertical level and specified x range
  temp_data_subset <- temp_data_4D[x_indices, , 1, ]
  
  # Calculate spatial averages at each time step and convert from K to C
...
min_y <- min(sapply(results_list, function(df) min(df$temp_mean, na.rm = TRUE)))
max_y <- max(sapply(results_list, function(df) max(df$temp_mean, na.rm = TRUE)))
x_range <- c(0, max_x)
...
for (i in 1:length(results_list)) {
  ...
  lines(data_to_plot$time_hr, data_to_plot$temp_mean,
        col = plot_colors[i],
        lwd = 2
  )
}
