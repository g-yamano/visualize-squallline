###################################################
# Title: visualize multiple PW time series Rscript 
# Author: Gaku YAMANO
# Date: 2025/08/26
###################################################
# load packages
...
min_y <- min(sapply(results_list, function(df) min(df$pw_mean, na.rm = TRUE)))
max_y <- max(sapply(results_list, function(df) max(df$pw_mean, na.rm = TRUE)))
x_range <- c(0, max_x)
...
for (i in 1:length(results_list)) {
  ...
  lines(data_to_plot$time_hr, data_to_plot$pw_mean,
        col = plot_colors[i],
        lwd = 2
  )
}
