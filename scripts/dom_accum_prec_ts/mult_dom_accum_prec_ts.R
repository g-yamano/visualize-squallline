###################################################
# Title: visualize multiple dom_ave_accum_prec time series Rscript 
# Author: Gaku YAMANO
# Date: 2025/08/26
###################################################
# load packages
...
for (experiment_name in names(input_files)) {
  ...
  results_list[[experiment_name]] <- data.frame(
    time_hr = alltimes_hr,
    accum_prec = dom_ave_accum_prec_timeseries
  )
}
...
max_y <- max(sapply(results_list, function(df) max(df$accum_prec, na.rm = TRUE)))
x_range <- c(0, max_x)
y_range <- c(0, max_y)
...
plot(NA, NA,
     ...
     xlim = x_range,
     ylim = y_range,
     main = "Domain Average of Accumulated Precipitation Time Series",
     xlab = "Time [hour]",
     ylab = "Accumulated Precipitation [kg/m^2/6hr]",
     yaxt = "n"
)
...
for (i in seq_along(results_list)) {
  ...
  lines(data_to_plot$time_hr, data_to_plot$accum_prec,
        col = plot_colors[i],
        lwd = 2
  )
}
