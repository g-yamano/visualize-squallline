###################################################
# Title: visualize multiple dom_max_prec_rate time series Rscript 
# Author: Gaku YAMANO
# Date: 2025/08/26
###################################################
# load packages
...
for (experiment_name in names(input_files)) {
  ...
  results_list[[experiment_name]] <- data.frame(
    time_hr = alltimes_hr,
    max_prec_rate = dom_max_prec_rate_timeseries
  )
}
...
max_y <- max(sapply(results_list, function(df) max(df$max_prec_rate, na.rm = TRUE)))
x_range <- c(0, max_x)
y_range <- c(0, max_y)
...
for (i in 1:length(results_list)) {
  ...
  lines(data_to_plot$time_hr, data_to_plot$max_prec_rate,
        col = plot_colors[i],
        lwd = 2
  )
}
