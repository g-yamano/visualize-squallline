###################################################
# Title: visualize multiple LWP time series Rscript 
# Author: Gaku YAMANO
# Date: 2025/08/26
###################################################
# load packages
...
max_y <- max(sapply(results_list, function(df) max(df$lwp_mean, na.rm = TRUE)))
x_range <- c(0, max_x)
...
for (i in 1:length(results_list)) {
  ...
  lines(data_to_plot$time_hr, data_to_plot$lwp_mean,
        col = plot_colors[i],
        lwd = 2
  )
}
